module.exports = function(grunt) {
    ...
    grunt.initConfig({
            ...
            def: {
                files: {
                    ...
                    '<%= pkg.assetsPath %>js/<%= pkg.name %>.js': [
                        ...
                        'node_modules/foundation-sites/dist/foundation.interchange.js',
                        'node_modules/foundation-sites/dist/foundation.reveal.js',
        ...
        compass: {
            options: {
                ...
                relativeAssets : true,
                importPath     : ['node_modules', 'node_modules/foundation-sites/scss/'],
                force          : true
            },
        ...
        watch: {
            scripts: {
                files: ['<%= pkg.buildPath %>js/app/**/*.js', '<%= pkg.buildPath %>js/lib/**/*.js'],
                tasks: ['jshint', 'concat'],
                options: {
                    livereload: true,
                }
            ...
            assemble: {
				...
				tasks: [ 'env:dev', 'assemble', 'preprocess'],
                options: {
                    livereload: true,
                }
			}
    ...
    // Default task(s).
    grunt.registerTask('default', ['env:dev', 'jshint', 'concat', 'compass:development', 'css_purge', 'postcss:development', 'assemble', 'preprocess', 'replace']);
    grunt.registerTask('browser', ['default', 'browserSync', 'watch']);
    grunt.registerTask('watcher', ['watch']);
    grunt.registerTask('production', ['env:prod', 'jshint', 'concat', 'uglify', 'compass:production', 'postcss:production', 'assemble', 'preprocess', 'replace', 'css_purge', 'cssmin']);
